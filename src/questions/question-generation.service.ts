...
export class QuestionGenerationService {
  ...
  private buildPromptFromLesson(
    ...
    for (const rule of lesson.grammarRules ?? []) {
      ...
      if (rule.structurePattern)
        parts.push(`Structure: ${rule.structurePattern}`);
  ...
  async generateQuestions(lessonId: string, count: number) {
    ...
    if (!lesson.module) {
      throw new InternalServerErrorException(
        `Lesson '${lessonId}' has no associated module`,
      );
    }
    ...
    if (!apiKey) {
      throw new InternalServerErrorException(
        "Gemini API configuration is missing (API_KEY)",
      );
    }

    const questionType = this.mapCategoryToQuestionType(
      lesson.module.slug,
      lesson.module.categoryType,
    );
  ...
  private async callGemini(
    ...
    if (!res.ok) {
      const snippet =
        rawText.length > 500
          ? rawText.slice(0, 500) + "...[truncated]"
          : rawText;
      throw new InternalServerErrorException(
        `Gemini API returned ${res.status}: ${snippet}`,
      );
    }
    ...
    if (!generatedText) {
      throw new InternalServerErrorException(
        "Gemini API response missing expected candidates/content/parts structure",
      );
    }
  ...
  private parseGeneratedQuestions(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        `Provider returned unparsable JSON: ${text.slice(0, 500)}`,
      );
    }

    const questions = Array.isArray(parsed)
      ? parsed
      : (parsed.questions ?? parsed.data ?? null);

    if (!Array.isArray(questions)) {
      throw new InternalServerErrorException(
        `Expected array of questions, got: ${JSON.stringify(parsed).slice(0, 500)}`,
      );
    }

    return questions.map((q, i) =>
      this.validateAndNormalize(q, i, fallbackType),
    );
  }
  ...
  private validateAndNormalize(
    ...
    if (typeof q.content !== "string" || !q.content.trim()) {
      throw new InternalServerErrorException(
        `Question ${index}: missing or invalid 'content'`,
      );
    }
    if (
      ...
      throw new InternalServerErrorException(
        `Question ${index}: 'options' must be an array of strings`,
      );
    }
    if (
      ...
      throw new InternalServerErrorException(
        `Question ${index}: 'correctAnswer' must match one of the options`,
      );
    }

    const questionType = Object.values(QuestionType).includes(q.questionType)
      ? q.questionType
      : fallbackType;
