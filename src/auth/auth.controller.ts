...
import {
  clearAuthCookies,
  setAuthCookies,
} from "src/common/utils/cookie.helpers";
...
export class AuthController {
  ...
  @ApiBody({
    schema: {
      type: "object",
      properties: {
        email: { type: "string", example: "admin@mokushi.com" },
  ...
  @ApiResponse({
    status: 200,
    description: "User logged in successfully with cookies set",
  })
  @ApiResponse({ status: 401, description: "Invalid credentials" })
  async login(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    const result = await this.authService.generateTokensAndSave(
      (req.user as any).id,
    );
  ...
  @Post("register")
  @ApiOperation({
    summary: "Register a new user and return JWT token in cookies",
  })
  @ApiBody({ type: CreateUserDto })
  @ApiResponse({
    status: 201,
    description: "User registered successfully with cookies set",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Tokens revoked successfully and cookies cleared",
  })
  ...
  async googleAuthCallback(@Req() req: Request, @Res() res: Response) {
    try {
      ...
      const errorRedirectUrl =
        process.env.FRONTEND_ERROR_REDIRECT_URL ||
