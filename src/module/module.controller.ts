...
import { CancelQuizDto } from "../user-attempts/dto/cancel-quiz.dto";
...
export class ModuleController {
  ...
  @Get(":id/quiz-configs")
  @ApiOperation({
    summary: "Get all available quiz configurations for a module",
  })
  ...
  @ApiOperation({
    summary: "Create a new quiz configuration for a module (Admin only)",
  })
  @ApiCreatedResponse({
  ...
  @ApiForbiddenResponse({ description: "Forbidden" })
  @ApiNotFoundResponse({ description: "Quiz config not found" })
  async deleteQuizConfig(@Param("configId") configId: string) {
  ...
  @ApiOperation({
    summary:
      "Start a quiz for a module with selected configuration (creates a UserAttempt automatically)",
  })
  @ApiOkResponse({
    description: "Quiz started with questions and timing info",
    type: ModuleQuizResponseDto,
  })
  ...
  @ApiQuery({
    ...
    required: true,
    type: String,
    description: "ID of the quiz configuration to use",
  })
  async getModuleQuiz(
    @Param("id") id: string,
    ...
    const deadline = new Date(
      startedAt.getTime() + quiz.quiz.durationSec * 1000,
    );
  ...
  @ApiOkResponse({
    ...
    schema: { example: { success: true, score: 8 } },
  })
  @ApiUnauthorizedResponse({ description: "Unauthorized" })
  async submitQuiz(
    @CurrentUser() user: any,
    ...
    const submission = await this.userAttemptsService.submitQuizAnswers(
      body.userAttemptId,
      body.answers,
    );
  ...
  @ApiBadRequestResponse({
    description: "Cannot cancel a completed quiz attempt",
  })
  @ApiOkResponse({
    description: "Quiz attempt cancelled successfully",
    schema: {
      example: {
        success: true,
  ...
  async cancelQuiz(
    @CurrentUser() user: any,
    @Body(ValidationPipe) body: CancelQuizDto,
  ) {
    return this.userAttemptsService.cancelAttempt(body.userAttemptId, user.id);
  }
